...
`08` 定义当前可执行 MVP 契约，`10-product-workflow-iteration-log.md` 记录产品树发散、取舍和后续候选。二者关系如下：

- 标记为 `MVP 保留` 或 `MVP 收敛` 的想法，必须同步回本文档的状态机、Widget 协议或系统动作边界后，才算进入实现基准。
- 标记为 `后续候选` 的想法可以出现在工作流说明页，但不得成为当前 MVP 的依赖。
- 标记为 `暂不做` 或 `已砍掉` 的内容不得出现在真实产品演示主路径。
- 当 `10` 中的新迭代影响状态机、Widget 清单、结构化记录或 AI/系统边界时，应优先更新本文档，再更新原型。
