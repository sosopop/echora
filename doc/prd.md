...
- **辅助追问**：普通对话一旦中途插话就会被带偏方向，辅助追问解决了这个问题——用户可以在主学习流不变的前提下，从 AI 批改卡片打开右侧支线深入追问，问完回到主线继续
...
- **`scene-select`**：AI 生成 100 个候选场景主题作为内部候选池，系统去重、排序后展示 8 个推荐场景卡片（已用队列最大 10），前端额外展示第 9 个自定义场景入口；用户自由输入主题时直接按该主题生成场景对话并进入练习。画像完成后自动触发。
...
2. 系统基于已用队列、难度匹配和主题多样性筛选 8 个推荐场景卡片展示给用户；若候选不足，使用确定性兜底主题补齐，避免九宫格缺格
3. 用户选择某个场景后，系统将该主题记入"已使用场景队列"（每用户最大容量 10）
4. 用户点击"换一批"时，系统从当前候选池或新候选池中重新补齐 8 个推荐卡片；生成新候选池时，prompt 明确排除已用队列中的主题
...
- 用户可从 AI 批改结果卡片打开右侧支线追问；普通对话气泡不提供追问按钮，避免把主线闲聊入口和卡片解释入口混在一起
- 继承源上下文，可连续追问，形成解释链（如"为什么用 on 不用 in？" -> "那 in the morning 为什么是 in？"）
- 从批改卡片发起时,`source_ref` 必须携带对应情景对话上下文、AI 提出的问题、用户回答、参考表达、AI 批改解析与错误标签
- 错误批改卡片基于用户回答、参考表达和错误标签展示 2-3 个默认追问；点击默认追问会直接在右侧支线发出该问题，点击通用"追问"按钮则只打开支线供用户手动输入
- **不能**改变主学习流状态、生成下一题或替用户提交答案——主线始终不受干扰
- 答题未提交前，辅助追问只提供提示和概念解释，不展示当前题标准答案、完整翻译或等价答案
- 默认不计入薄弱点统计；只有用户明确选择"加入复盘"时才写入结构化记录
- 支线消息复用 `messages` 表，通过 `branch_thread_id` 归属到对应辅助追问线程；`source_ref` 指向批改卡片 Widget 和相关 attempt/grading 上下文
...
- **`scene-cards`**：场景推荐卡片组（3x3 九宫格，8 个推荐卡 + 第 9 个自定义入口；每张含 emoji + 标题 + 知识点 + 难度），hover 轻微上浮
- **`exercise-card`**：练习题主卡片（题干 + 上下文 + 答题入口）
- **`fill-blank`**：填空输入区域
- **`choice-question`**：选择题按钮组
- **`grading-result`**：批改结果卡片（顶部状态条颜色区分对错、用户答案 vs 参考答案对比、中文解析、中文错误标签 chips、默认追问按钮和手动追问入口）
...
- **场景选择**：系统每次展示 8 个推荐场景卡片，前端额外展示 1 个自定义入口；用户最近 10 个已用主题不得重复展示
