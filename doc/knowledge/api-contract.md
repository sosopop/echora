...
065 起,前端不再给普通主线消息气泡显示追问按钮,追问入口只放在 `grading-result` 卡片上。批改卡片创建支线时,`sourceRef.kind='grading-result'`,并携带 `widgetId/attemptId/scenarioContext/aiQuestion/myAnswer/referenceAnswer/aiAnalysis/tags`。这类来源是已提交后的批改结果,即使当前主线已进入下一题的 locked 状态,支线 Provider prompt 也会携带这份结构化批改上下文;未批改题或普通消息来源仍按锁定规则隐藏正文。067 起错误批改卡片会在前端基于 `tags/userAnswer/referenceAnswer/explanation` 生成 2-3 个默认追问按钮;点击默认追问等价于先创建/打开该批改卡片支线,再调用 `/api/chat/branch-threads/:threadId/messages` 发送对应文本。
