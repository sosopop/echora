...
- **ProgressSummary 组件(015)**:`src/components/widgets/ProgressSummary.tsx` 正式渲染 `progress-summary`,展示题数/平均分/薄弱点/达标项、掌握度条、强弱项与静态建议。`status='loading'` 或缺少 `questionsCount/averageScore` 时返回 `null`,不走 fallback JSON。
