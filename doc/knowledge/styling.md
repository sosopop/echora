...
- 066/067 起 Chat 顶栏账号菜单承载主题三段切换与退出登录;头像入口为可聚焦 button,菜单内显示账号摘要、亮色/暗色/系统三选一和危险色退出按钮。
...
- **Chat 停止生成(041/051)**:`src/views/Chat/ChatInput.tsx` 在 `streamingMessageId` 存在时把发送按钮切换为"停止";点击后调用 `useChatStore.stopGenerating()` → `POST /api/chat/streams/:streamId/abort`,关闭本地 fetch SSE 流并清空流式态。若 assistant 尚无内容,前端写入"已停止生成。"避免空回复;后端会把 abort 贯穿到 provider route/chat 与长运行 skill helper。
...
- **批改卡片 loading 与三档结果(009/021/065/067)**:`grading-result` 在 `status='ready'` 且有 `category(exact/similar/incorrect)` 或历史 `isCorrect` 前不渲染结果卡,避免先出现占位结果;等待态由 assistant 文本承载。021 起批改卡不展示百分制分数,只展示"完全正确 / 还不错 / 错误"三档,且正确或相近时不再显示"下一题"按钮,由后端自动串接下一题。065 起错误标签 chip 使用中文展示,批改卡底部显示唯一的主线追问入口。067 起错误卡片按标签/答案生成 2-3 个默认追问按钮,点击后打开右侧支线并直接发送该追问;手动"追问"按钮只打开支线输入。
- **场景卡片 loading(011/063)**:`scene-cards` 在 `status='loading'` 时不渲染空候选小部件,只保留 assistant 文本;`ready` 后一次性出现 8 个推荐卡 + 1 个自定义入口,`error` 时才显示恢复提示。
- **场景九宫格(063)**:`src/components/widgets/SceneCards.tsx` 采用 3 列九宫格布局,桌面固定 3x3 视觉节奏,窄屏降为 2 列或 1 列;卡片固定最小高度并限制标题/描述换行,避免 emoji、标题和知识点拥挤跳动。
...
- **ProgressSummary 组件(015/016/029/065)**:`src/components/widgets/ProgressSummary.tsx` 正式渲染 `progress-summary`,展示题数、三档分布(完全正确/还不错/错误)、薄弱点、掌握度条、强弱项与建议。016 起建议卡片有"开始"按钮:`retry:<tag>` 转成文本 `重练 <tag>`,`request-new-scenes` 继续走结构化 action。029 起不展示平均分;065 起错误标签展示中文但按钮 action 保留英文 tag;`status='loading'` 或缺少 `questionsCount/averageScore` 时返回 `null`,不走 fallback JSON。
...
- **AnswerReview 组件(017/065)**:`src/components/widgets/AnswerReview.tsx` 正式渲染 `answer-review`,展示逐题短题干、分数 badge、题型和中文错误标签。`status='loading'` 或 items 为空时返回 `null`。017 起 `MessageList` 支持同一 assistant 消息多个 widget snapshot,用于复盘总览 + 单题回看连续呈现。
...
- **辅助追问右侧面板(031/044/065/067)**:`src/views/Chat/BranchPanel.tsx` 渲染桌面右侧支线;065 起普通主线消息气泡不显示"追问"入口,只有 AI 批改卡片内的追问按钮会打开支线,并用 `sourceRef.kind='grading-result'` 传递题目上下文、用户答案和批改解析。067 起支线 assistant 回复使用轻量 Markdown 渲染,支持段落、加粗、列表、引用、代码和链接。当前追问来源用 `bubbleReferenced` 高亮。桌面宽屏下 `shellWithBranch` 把主区与 360px 支线组成两列,并把固定输入栏右侧收进主区;窄屏下支线用 fixed 面板覆盖,主输入栏暂时隐藏,避免两套输入重叠。044 起,已批改且带错误标签的来源会显示"加入复盘"按钮,成功后在来源块内显示确认文案。
...
- 主题切换通过手工烟雾验证:启动 dev:web → 打开账号菜单 → 切换亮色/暗色/系统 → `localStorage.echora-theme` 被写入或清除
- Chat 滚动行为:`src/__tests__/views/MessageList.test.tsx`
- Chat 消息顺序/思考占位/Markdown 渲染:`src/__tests__/stores/chat.test.ts` + `src/__tests__/views/MessageBubble.test.tsx`
...
- 辅助追问入口、默认追问、Markdown 回复、加入复盘按钮与支线 store:`src/__tests__/views/MessageList.test.tsx` + `src/__tests__/views/BranchPanel.test.tsx` + `src/__tests__/stores/chat.test.ts`
