...
- **批改卡片 loading 与三档结果(009/021/065)**:`grading-result` 在 `status='ready'` 且有 `category(exact/similar/incorrect)` 或历史 `isCorrect` 前不渲染结果卡,避免先出现占位结果;等待态由 assistant 文本承载。021 起批改卡不展示百分制分数,只展示"完全正确 / 还不错 / 错误"三档,且正确或相近时不再显示"下一题"按钮,由后端自动串接下一题。065 起错误标签 chip 使用中文展示,批改卡底部显示唯一的主线追问入口。
...
- **辅助追问右侧面板(031/044/065)**:`src/views/Chat/BranchPanel.tsx` 渲染桌面右侧支线;065 起普通主线消息气泡不显示"追问"入口,只有 AI 批改卡片内的追问按钮会打开支线,并用 `sourceRef.kind='grading-result'` 传递题目上下文、用户答案和批改解析。当前追问来源用 `bubbleReferenced` 高亮。桌面宽屏下 `shellWithBranch` 把主区与 360px 支线组成两列,并把固定输入栏右侧收进主区;窄屏下支线用 fixed 面板覆盖,主输入栏暂时隐藏,避免两套输入重叠。044 起,已批改且带错误标签的来源会显示"加入复盘"按钮,成功后在来源块内显示确认文案。
